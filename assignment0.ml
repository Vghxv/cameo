...
let nb_bit_pos n =
  let rec aux n acc =
    match n with 0 -> acc | _ -> aux (n / 2) (acc + (n mod 2))
  in
  aux n 0

(* 2 *)
let fibo n =
  let rec aux n a b = match n with 0 -> a | _ -> aux (n - 1) b (a + b) in
  aux n 0 1
...
let isPalindrome l = match l with "" -> true | _ -> l = reverse l
...
let rec comparel l1 l2 =
  ...
  | _, "" -> true
  | _ ->
      if l1.[0] < l2.[0] then false
      else if l1.[0] > l2.[0] then true
      else
        comparel
          (String.sub l1 1 (String.length l1 - 1))
          (String.sub l2 1 (String.length l2 - 1))
...
let rec square_sum l =
  match l with [] -> 0 | x :: tl -> (x * x) + square_sum tl
...
let find_opt x l =
  let rec aux index l =
    match l with
    | [] -> None
    | y :: tl -> if y = x then Some index else aux (index + 1) tl
  in
  aux 0 l
...
let find_opt_no_rec x l =
  let _, result =
    List.fold_left
      (fun (index, acc) y ->
        if y = x then (index, Some index) else (index + 1, acc))
      (0, None) l
  in
  result
...
let rev l =
  let rec aux acc l = match l with [] -> acc | x :: tl -> aux (x :: acc) tl in
  aux [] l

let map f l =
  let rec aux acc l =
    match l with [] -> acc | x :: tl -> aux (f x :: acc) tl
  in
  aux [] l

(* 7 *)
type 'a seq = Elt of 'a | Seq of 'a seq * 'a seq

let ( @@ ) s1 s2 = Seq (s1, s2)

(* 7-a *)
let hd_ s =
  let rec aux s = match s with Elt x -> x | Seq (s1, _) -> aux s1 in
  aux s

let tl_ s =
  let rec aux s = match s with Elt x -> x | Seq (_, s2) -> aux s2 in
  aux s
...
let rec rev_ s =
  match s with Elt x -> Elt x | Seq (s1, s2) -> Seq (rev_ s2, rev_ s1)

let rec map_ f s =
  match s with Elt x -> Elt (f x) | Seq (s1, s2) -> Seq (map_ f s1, map_ f s2)

let rec fold_left_ f acc s =
  match s with
  | Elt x -> f acc x
  | Seq (s1, s2) -> fold_left_ f (fold_left_ f acc s1) s2

let rec fold_right_ f s acc =
  match s with
  | Elt x -> f x acc
...
let rec seq2list_ s =
  let rec aux acc s =
    match s with Elt x -> x :: acc | Seq (s1, s2) -> aux (aux acc s2) s1
  in
  aux [] s
...
let find_opt_ x s =
  ...
  and count s =
    match s with Elt _ -> 1 | Seq (s1, s2) -> count s1 + count s2
  in
  aux 0 s
...
let q description f exp =
  let result = f () in
  Printf.printf "%s: %s : %s\n" description result
    (if result = exp then "Passed" else "Failed")

let o2s x = match x with None -> "" | Some x -> string_of_int x
...
let () =
  ...
  q "factor abc 123abc456"
    (fun () -> string_of_bool (factor "abc" "123abc456"))
    "true";
  q "factor xyz 123abc456"
    (fun () -> string_of_bool (factor "xyz" "123abc456"))
    "false";
  let l = [ 1; 3; 2; 4; 5; 7; 6 ] in
  let l' = mergesort l in
  q "mergesort"
    (fun () -> List.fold_left (fun acc x -> acc ^ string_of_int x ^ " ") "" l')
    "1 2 3 4 5 6 7 ";
  let l = [ 1; 3; 2; 4; 5; 7; 6 ] in
  ...
  let l = [ 1; 3; 2; 4; 5; 7; 6 ] in
  q "find_opt_no_rec 5"
    (fun () -> string_of_int (Option.get (find_opt_no_rec 7 l)))
    "5";
  let l = [ 1; 3; 2; 4; 5; 7; 6 ] in
  ...
  let s = Seq (Elt 1, Seq (Elt 2, Seq (Elt 3, Elt 4))) in
  q "fold_left_"
    (fun () -> string_of_int (fold_left_ (fun acc x -> acc + x) 0 s))
    "10";
  let s = Seq (Elt 1, Seq (Elt 2, Seq (Elt 3, Elt 4))) in
  q "fold_right_"
    (fun () -> string_of_int (fold_right_ (fun x acc -> x + acc) s 0))
    "10";
  let s = Seq (Elt 1, Seq (Elt 2, Seq (Elt 3, Elt 4))) in
  q "seq2list_"
    (fun () ->
      List.fold_left (fun acc x -> acc ^ string_of_int x ^ " ") "" (seq2list_ s))
    "1 2 3 4 ";
  let s = Seq (Elt 1, Seq (Elt 2, Seq (Elt 3, Elt 4))) in
